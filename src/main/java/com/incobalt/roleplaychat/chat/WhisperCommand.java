...
/*
    WhisperCommand is the command class that registers /whisper (and /w if vanilla commands are replaced).
    /whisper sends a message to very nearby players within a configurable range with an emotive bleed out to one additional block
    ...
    WhisperCommand extends CommandBase, which means it must implement a constructor and registerCommand()
...
public class WhisperCommand extends CommandBase {
    ...
    public WhisperCommand(ArrayList<String> a) {
    ...
    public void registerCommand(CommandDispatcher<CommandSource> dispatcher){
        ...
        CoreAPI.LOGGER.info("Registering Whisper Command");
        ...
        for(String a : aliases) {
            ...
            //Commands.literal defines what the player types after the / (whisper for /whisper)
            ...
            LiteralArgumentBuilder<CommandSource> commandLiteral = Commands.literal(a)
                    .then(Commands.argument("message", MessageArgument.message())
                            .executes( ctx ->{
                                ...
                                ChatCommands.ProcessWhisper(message, source.getDisplayName().getFormattedText(), pos, source.getWorld());
