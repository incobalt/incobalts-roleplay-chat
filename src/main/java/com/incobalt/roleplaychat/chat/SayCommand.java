...
/*
    SayCommand is the command class that registers /say if vanilla commands are replaced.
    /say sends a message to nearby players within a configurable range
    ...
    SayCommand extends CommandBase, which means it must implement a constructor and registerCommand()
...
public class SayCommand extends CommandBase {
    ...
    public SayCommand(ArrayList<String> a) {
    ...
    public void registerCommand(CommandDispatcher<CommandSource> dispatcher){
        ...
        CoreAPI.LOGGER.info("Registering Say Command (overwriting!)");
        ...
        for(String a : aliases) {
            ...
            //Commands.literal defines what the player types after the / (say for /say)
            ...
            LiteralArgumentBuilder<CommandSource> commandLiteral = Commands.literal(a)
                    .then(Commands.argument("message", MessageArgument.message())
                            .executes( ctx ->{
                                ...
                                ChatCommands.ProcessDistanceSay(message, source.getDisplayName().getFormattedText(), pos, source.getWorld());
